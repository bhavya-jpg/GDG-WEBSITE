...
// GDG Theme Colors (Google brand colors)
const defaultColors = ['#4285F4', '#EA4335', '#FBBC04', '#34A853'];
...
`;
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(props?: { cursor_size?: number; mouse_force?: number; cellScale?: THREE.Vector2 }) {
                const { cursor_size, mouse_force, cellScale } = props || {};
                const forceX = (Mouse.diff.x / 2) * (mouse_force || 0);
                const forceY = (Mouse.diff.y / 2) * (mouse_force || 0);
                const cs = cellScale || new THREE.Vector2(1, 1);
                const cursorSize = cursor_size || 0;
                const cursorSizeX = cursorSize * cs.x;
                const cursorSizeY = cursorSize * cs.y;
